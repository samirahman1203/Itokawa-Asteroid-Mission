...
THETA_values = zeros(nsteps_i, nsteps_j); % Array for THETA values
DECLINATION_values = zeros(nsteps_i, nsteps_j); % Array for declination (end_theta)
...
for i = 1:nsteps_i
    ...
    for j = 1:nsteps_j    
        ...
        try
            ...
            % Finds declination on arrival
            Vf_magnitude = abs(Vi);      % Magnitude of final velocity vector
            Vf_z = abs(Viz);                 % Z-component of final velocity vector
            sin_delta = Vf_z / Vf_magnitude; % sin(delta) = VF_z / ||VF||
            end_theta = asin(sin_delta);  % Declination angle in radians
            DECLINATION_values(j, i) = end_theta * (180 / pi); % Convert to degrees
        catch
            dv(i, j) = NaN; % Assign NaN if the Lambert solver fails
            THETA_values(j, i) = NaN;
            DECLINATION_values(j, i) = NaN;
        end
    end
end
...
figure;
contourf(ti, ToF, THETA_values, 40, 'ShowText', 'on');
colorbar;
xlabel('Departure Time (MJD)');
ylabel('Time of Flight (days)');
title('Theta plot in (Degrees) for Pork-Chop Plot outward');
set(gca, 'YDir', 'normal');


% Plot Declination for pork chop TOF
figure;
contourf(ti, ToF, DECLINATION_values, 40, 'ShowText', 'on');
colorbar;
xlabel('Departure Time (MJD)');
ylabel('Time of Flight (days)');
title('Declination (Degrees) for Arrival at Itokawa');
set(gca, 'YDir', 'normal');
...
xlabel('Departure Time (MJD)');
ylabel('Time of Flight (days)');
title('Theta plot in (Degrees) for Pork-Chop Plot return');
set(gca, 'YDir', 'normal');
...
xlabel('Departure Time (MJD)');
ylabel('Time of Flight (days)');
title('Declination (Degrees) for Arrival at Earth ');
set(gca, 'YDir', 'normal');
