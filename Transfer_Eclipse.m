...
MuE = 3.986004418e5; % Gravitational parameter of the Earth (Km^3/s^2)
au = 1.496e11; % Astronomical Unit in meters

% Launch parameters
launch_date = datetime(2036, 3, 16);
transfer_time_days = 345 + 7; % 
...
[t, state] = ode45(@(t, y) two_body_equation(t, y, MuS), [0, transfer_time_days * 86400], y0, options); % Adjust time span for 101-day transfer
...
for i = 1:length(t)
    ...
    r_earth = r_earth_sun(:, ephem_index); % Get Earth position at the closest available ephemeris time step
...
yticks([0 1]);
xlabel('Time (days)');
title('Eclipse Status during Spacecraft Transfer to Itokawa');
...
figure;
plot(days_array, eclipse_hours, '-o', 'LineWidth', 2);
xlabel('Day Number');
ylabel('Eclipse Duration (hours)');
title('Eclipse Duration per Day for Spacecraft Transfer to Itokawa');
grid on;
grid on;
